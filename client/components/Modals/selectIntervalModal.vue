  ...
  <div class="modal_window">
    <div class="modal_window_i">
      <div class="mw_up modal-event-header">
        <span class="modal-event-title">Выделить события в интервале</span>
        <img class="close_btn" @click="close" src="../../static/svg/close.svg" alt="Nope">
      </div>
      <div class="modal-event-content ">
        <label style="margin-top: 40px;">Укажите с какой до какой секунды в сценарии выделить события</label>
        <div class="content-row slider-wrapper" >
          <div>
            <label>С</label>
            <span style="font-size: 14px;">{{ fromTimeshift}}</span>
          </div>
          <vue-slider v-model="fromValSlider" :min="fromMin" :max="fromMax" :height="6"
                      @change="changeFromSlider"
        ...
        <div class="content-row slider-wrapper" >
          <div>
            <label>До</label>
            <span style="font-size: 14px;">{{ toTimeshift}}</span>
          </div>
          <vue-slider v-model="toValSlider" :min="toMin" :max="toMax" :height="6"
                      @change="changeToSlider"
                      :barStyle="{ backgroundColor: '#00ff00' }"
                      :dotStyle="{ backgroundColor: '#007bff' }"
                      :processStyle="{ backgroundColor: '#007bff' }"
          />
...
export default {
  ...
  methods: {
    save() {
      this.saveModal(this.fromValSlider, this.toValSlider)
    },
    close() {
      this.closeModal()
    },
    changeFromSlider() {
      this.fromTimeshift = ''
      let tmpTime1 = Math.floor(this.fromValSlider / 3_600)
      let tmpTime2 = this.fromValSlider % 3_600
      this.fromTimeshift += (tmpTime1 === 0 ? '' : tmpTime1 + ' ч ' )

      tmpTime1 = Math.floor(tmpTime2 / 60)
      tmpTime2 = tmpTime2 % 60
      this.fromTimeshift += (tmpTime1 === 0 ? '' : tmpTime1 + ' мин ' )

      this.fromTimeshift += (tmpTime2 === 0 ? '' : tmpTime2 + ' сек ' )

      if (this.fromTimeshift !== '') {
        this.fromTimeshift = '+' + this.fromTimeshift
      }
    },
    changeToSlider() {
      this.toTimeshift = ''
      let tmpTime1 = Math.floor(this.toValSlider / 3_600)
      let tmpTime2 = this.toValSlider % 3_600
      this.toTimeshift += (tmpTime1 === 0 ? '' : tmpTime1 + ' ч ' )

      tmpTime1 = Math.floor(tmpTime2 / 60)
      tmpTime2 = tmpTime2 % 60
      this.toTimeshift += (tmpTime1 === 0 ? '' : tmpTime1 + ' мин ' )

      this.toTimeshift += (tmpTime2 === 0 ? '' : tmpTime2 + ' сек ' )

      if (this.toTimeshift !== '') {
        this.toTimeshift = '+' + this.toTimeshift
      }
    }
  }
}
