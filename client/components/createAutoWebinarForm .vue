                ...
                <div class="option">
                  <div class="option__webinar">
                    ...
                    <v-file-input
                      ...
                      :disabled="screensaverAudioFilename !== '' || screensaverPhoto !== null" 
                    />
                    <div v-if="screensaverVideoProgress > 0" class="progress">
                      <div class="progress-container">
                        <div class="progress-bar" :style="{ width: screensaverVideoProgress + '%' }"></div>
                      </div>
                      <div>{{ screensaverVideoProgress }}%</div>
                    </div> -->
                    <div style="margin-top: 20px" class="room_checkbox_column room__checkbox">
...
export default {
  ...
  methods: {
    ...
    async createWebinar() {
      ...
      // let screensaverVideoFilename = null 
      // if (this.screensaverVideo) {
      //   screensaverVideoFilename = await this.uploadPhoto(this.screensaverVideo)
      // }
      ...
      // if (this.source) {
      //   sourceFilename = await this.uploadPhoto(this.source)
      // }
      ...
      try {
        const webinarData = {
          ...
          status: 0,
          userCount: 0,
          duration: this.$store.state.autowebinarDuration,
          videoType: 0,
        }
        if (this.banWordsOn) {
    ...
    async onFileChange(file, field) {
      if (!file) {
        if (field === 'userAvatarUrl') {
          ...
          return
        } else if (field === 'screensaverVideoUrl') {
          this.screensaverVideo = file
          this.screensaverVideoProgress = 0
          this.screensaverVideoFilename = ''
          return
        ...
        return;
      } else if (field === 'screensaverVideoUrl' || field === 'sourceUrl') {
        const totalChunks = Math.ceil(file.size / this.chunkSize)
        let uploadedChunks = 0
        let mergedFilename = ''
        let fileName = file.name.split('.')[0]
        if (field === 'screensaverVideoUrl') {
          this.screensaverVideoProgress = 3
        } else if (field === 'sourceUrl') {
        ...
        for (let start = 0; start < file.size; start += this.chunkSize) {
          ...
          if (field === 'screensaverVideoUrl') {
            this.screensaverVideoProgress = Math.round((uploadedChunks / totalChunks) * 100)
            // if ((uploadedChunks - 1) === totalChunks) {
              this.screensaverVideoFilename = mergedFilename
            // }
          } else if (field === 'sourceUrl') {
            ...
            if (this.sourceProgress === 100) {
              ...
              console.log("encodeeeeeeeeeeeeeeeeee", this.sourceFilename)
            }
            // }
          }
        }
