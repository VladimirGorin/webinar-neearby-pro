...
import AudioControl from "~/components/Modals/audioControl.vue"
    ...
    this.$axios.get(`/webinars/prettyLink?prettyLink=${webinarSearch}&isAutowebinar=${isAutowebinar}`)
      .then(data => {
        ...
        if (isSecret == 0 && localStorage.getItem("token")) {
          ...
          localStorage.setItem("isAutoWebinar", 0);
        ...
        socket.onmessage = async (event) => {
          ...
          if (command.action == "authed") {
            this.videoPlayer = videojs('video-player', {
              ...
              sources: [
                {
                  src: 'https://hls.neearby.pro/'+data.data.source+'.m3u8',
            ...
            this.videoPlayer.on('error', function(err) {
              if (err) {
                this.errorDisplay.contentEl().innerHTML = ''
            ...
            if (this.screensaverPhoto) {
              ...
              console.log(localStorage.getItem('fingerprintId'))
              document.getElementById('screensaverVideo').style.display = 'block';
              document.getElementById('screen__overlay').style.display = 'block';
              let videoId = this.screensaverVideo.split('/')
              console.log(videoId)
              console.log(videoId[videoId.length - 1])
              rt = new Rutube();
            ...
            this.initTime = date.getTime() + (offset * 60000)
            socket.send(JSON.stringify({
              action: "join",
              data: {
                isAutowebinar: isAutowebinar,
  ...
  methods: {
    ...
    async start() {
      ...
      if (this.videoPlayer.error()) {
        console.log("first", this.videoPlayer)
        document.getElementById('video-player').style.display = 'none'
        this.videoPlayer.dispose()
        console.log("second", this.videoPlayer)
        document.getElementById('temp-video-player').style.display = 'block'
        this.videoPlayer = videojs('temp-video-player', {
          fill: true,
          fluid: false,
          sources: [
            {
              src: 'https://hls.neearby.pro/'+this.source+'.m3u8',
              type: 'application/x-mpegURL',
            },
          ],
        });
      }
      console.log("last", this.videoPlayer)
      this.videoPlayer.ready(() => {
        console.log('Video.js player is ready');
        this.triggerBigPlayButton();
      ...
      const autowebinarVideoSource = await this.$axios.get(`/files/recordFileName?streamKey=${this.source}`)
      console.log("autowebinarVideoSource", autowebinarVideoSource)
      this.autoWebinarSource = autowebinarVideoSource.data
    ...
    async leave() {
      ...
      if (this.isAdmin || this.isModer) {
        ...
        this.dateStartPole = "Вебинар закончен"
      } else {
        if (!this.redirectOut) {
          this.stopState = true
          if (this.screensaverPhoto) {
    ...
    async goEditWebinar() {
      ...
      this.$store.commit('setWebinarType', this.isAutowebinar);
