    ...
    socket.onmessage = async (event) => {
      ...
      if (command.action == "ping") {
        ...
        socket.send(JSON.stringify({
          ...
          data: {
            ...
            authData: auth.data,
            login: strLogin,
            chat: String(this.webinar.id),
            device: String(navigator.userAgent?.toLowerCase()),
            msg: this.webinar.comment,
      ...
      if (command.action == "authed") {
        socket.send(JSON.stringify({
          ...
          data: {
            isAutowebinar: this.isAutowebinar,
            chat: String(this.webinar.id), // this.$route.params.id
          }
        }))  
      }
      ...
      if (command.action == "updateRoom") {
        ...
        this.userStatus = command.data.roomData.userStatus
        this.author = command.data.roomData.userName
        this.title = command.data.roomData.title
        this.dateStart = command.data.roomData.dateStart
        this.dateStartPole = this.$moment(this.dateStart).format("DD.MM.YYYY в hh:mm мск")
        if (command.data.roomData.userAvatar !== '' && command.data.roomData.userAvatar !== null) {
          this.authorAvatar = this.$config.staticURL + '/' + command.data.roomData.userAvatar
        } else {
          this.authorAvatar = `/author__photo.png`
        }
        this.additionalLinkEnterRoom = command.data.roomData.additionalLinkEnterRoom
        this.status = command.data.roomData.status
        this.buttonEnteringPage = command.data.roomData.buttonEnteringPage
        this.redirectLeaveEnteringPage = command.data.roomData.redirectLeaveEnteringPage ? command.data.roomData.redirectLeaveEnteringPage : 'http://neearby.pro'

        if (command.data.roomData.backgroundImageStandard) {
          this.backgroundImage = {
            'background-image': `url('${this.$config.staticURL + '/' + command.data.roomData.backgroundImageStandard}')`,
            'background-size': '100% 100%',
          }
        }
        this.webinar = command.data.roomData
        this.interval()
      }
    }
  },
