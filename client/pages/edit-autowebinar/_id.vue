    ...
    <div class="main__content">
      ...
      <div class="content__createwebinar">
        ...
        <div class="webinar__autowebinar2"
          ...
          <div class="option__webinar">
            <label class="label__webinar" >Заголовок вебинара<span class="red--text">*</span>: <span class="room__subtitle">(не больше 140 символов)</span></label>
            <input class="input__option" v-model="webinarData.title" type="text" :maxlength="140">
          </div>
          <div class="title-wrapper">
            <div>
              <div class="option__webinar">
                <label class="label__webinar" >Ведущий вебинара<span class="red--text">*</span>: <span class="room__subtitle">(не больше 50 символов)</span></label>
                <input class="input__option" v-model="webinarData.userName" type="text" :maxLength="50">
              ...
              <div class="option__webinar">
                ...
                <input class="input__option" v-model="webinarData.userStatus" type="text" :maxLength="50">
              </div>
                ...
                <div class="customDiv">
                  <div style="width: 300px;">
                    <v-file-input 
                      ...
                      v-model="webinarData.userAvatar" 
              ...
              <div style="margin-top: 30px;" class="room__create chat-room-background">
                ...
                <div class="option">
                  <div class="option__webinar">
                    ...
                    <input class="input__option" type="text" v-model="webinarData.comment" :maxLength="300">
                  </div>
                  <hr style="margin-top: 20px;" />
                    ...
                    <div class="modal__content1" style="margin-top: 20px">
                      <div class="modals__input">
                        <div>
                          ...
                          <input class="input__option" type="text" v-model="newLinkName" :maxLength="50">
                        </div>
                        <div>
...
export default {
  name: 'AutoIndexPage',
  components: {ChangeMiniature, DeleteWebinar, CreateWebinar, Select2, DescriptionModal, Datetime},
  ...
  data() {
    return {
      ...
      screensaverVideoProgress: 0,
      sourceProgress: 0,
      screensaverAudioFilename: '',
      screensaverVideoFilename: '',
      source: '',
      sourceFilename: '',
      tempDate: '',
      displayDate: '',
  ...
  async mounted() { 
    ...
    if (!localStorage.getItem('token')) {
      ...
      if (this.strScriptEditor === 'edit') {
        const data = JSON.parse(JSON.stringify(this.$store.state.autowebinar));
        console.log(data)
        if (data != null) {
          ...
          this.customStandardBackground = this.$store.state.autowebinar.customStandardBackground
          this.screensaverPhoto = this.$store.state.autowebinar.screensaverPhoto
          this.screensaverAudio = this.$store.state.autowebinar.screensaverAudio
          this.webinarData.userAvatar = this.$store.state.autowebinar.webinarData.userAvatar
          this.source = this.$store.state.autowebinar.webinarData.source
          this.webinarData.source = this.$store.state.autowebinar.webinarData.source
          console.log("webbbb", this.webinarData, this.$store.state.autowebinar.webinarData.source)
        }
      } else {
        const params = {
          id: this.editId,
        ...
        for (const [key, value] of Object.entries(data)) {
          if (Object.hasOwn(this.webinarData, key)) {
            ...
            if (key === 'videoType') {
              this.webinarData.videoType = value
            }
      ...
      this.isRoomEdit = this.$store.state.isRoomEdit
      let auth = {
        type: "ghoste",
        data: "editUser",
      }
      ...
      socket.onmessage = async (event) => {
        ...
        if (command.action == "authed") {
          ...
          socket.send(JSON.stringify({
            ...
            data: {
              isAutowebinar: this.isAutowebinar,
              chat: String(this.webinarData.id), // this.$route.params.id
            }
          }))  
        }
        if (command.action == "addLink") {
          ...
          this.webinarData.links = [
            ...this.webinarData.links,
            {
              nameLink: command.data.nameLink,
              msgLink: command.data.msgLink,
              colorLink: command.data.colorLink,
            }
          ]
        }
      }
  ...
  methods: {
    ...
    async updateWebinar() {
      ...
      try {
        var temp = this.tempDate.split('.')
        this.webinarData.dateStart = temp[0].replace(":00", "")
    ...
    async onFileChange(file, field) {
      if (!file) {
        ...
        let mergedFilename = ''
        this.sourceProgress = 3
        let fileName = file.name.split('.')[0]
        for (let start = 0; start < file.size; start += this.chunkSize) {
          const chunk = file.slice(start, start + this.chunkSize)

          mergedFilename = await this.uploadChunk(file, chunk, start / this.chunkSize)
          console.log(mergedFilename)
          uploadedChunks++
          
          this.sourceProgress = Math.round((uploadedChunks / totalChunks) * 100)
          if (this.sourceProgress === 100) {
            this.loading = true
            const encodeInterval = setInterval(() => {
              if (this.spiningFlag % 3 === 0) {
                this.spining = '.'
              } else if (this.spiningFlag % 3 === 1) {
                this.spining = '..'
              } else {
                this.spining = '...'
              }
            ...
            const encode = await this.$axios.post(
              '/files/encode', 
              {
    ...
    async uploadChunk(file, chunk, chunkNumber) {
      const formData = new FormData()
      ...
      const res = await this.$axios.post('/files/video', formData, {
          headers: {
              'Content-Type': 'multipart/form-data' // Set standard headers only
          },
      });
      console.log(res)
      return res.data?.filename
    ...
    async goToScriptEditor() {
      console.log('go', this._data)
      this.$store.commit('setAutowebinar', this._data);
      await this.$router.push('/edit-an-autowebinar-script-editor')
    },
    toggleMainStr() {
