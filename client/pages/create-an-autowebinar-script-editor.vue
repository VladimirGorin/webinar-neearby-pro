...
export default {
  ...
  methods: {
    async initData() {
      ...
      const customSource =  process.env.STATIC_URL + '/' + this.autowebinar.sourceFilename
      const player = document.getElementById('video-player');
      const sourceElement = player.querySelector('source');
      sourceElement.src = customSource;
      if (this.autowebinar.videoType) {
        dashboardPlayer.src({ type: 'video/flv', src: customSource });
      } else {
        dashboardPlayer.src({ type: 'video/mp4', src: customSource });
      }
      dashboardPlayer.ready(function() {
        // const savedTime = localStorage.getItem('previewPlayerUpdateTime');
        // if (savedTime) {
        //   dashboardPlayer.currentTime(parseFloat(savedTime)); // Set the current time to the saved time
        // }
        dashboardPlayer.load()
        // Automatically play the video (optional, based on user interaction rules)
        // dashboardPlayer.play().catch(function(error) {
        //   console.error('Error playing the video:', error);
        // });
        // dashboardPlayer.on('timeupdate', function() {
        //   localStorage.setItem('previewPlayerUpdateTime', dashboardPlayer.currentTime());
        // });
        // dashboardPlayer.on('ended', function() {
        //   localStorage.removeItem('previewPlayerUpdateTime');
      ...
      const previewSource =  process.env.STATIC_URL + '/' + this.autowebinar.sourceFilename
      ...
      if (this.autowebinar.videoType) {
        previewPlayer.src({ type: 'video/flv', src: previewSource });
      } else {
